...
the position (and facing) of its bearer will be recorded with a blue marker,
...
so that the {{buff(b="return", short=true)}} expi[res 40 seconds after the start of the mechanic.
...
so the recorded positions (and facings) must solve these debuffs.
