...
# **NOTE: THIS IS AN EARLY DRAFT AND MAY BE INAKURATE AND INCOMPLET**
...
2. The wing attack is a massive 240° tankbuster cleave that will hit the entire third of the arena
   in front of {{mob(n="Pandora")}}, as well as the entire side of the corresponding wing.
   The safe zone is therefore anywhere behind the front third of the room,
   and on the {{light(t="right")}} or {{dark(t="left")}} side, depending on the wing/element.
   It's the same ginormous cleave that {{mob(n="Eden's Promise")}}
   ...
   will receive a {{buff(b="ddown", dur="2m", short=true)}}, so cheesing it is not an option.

   Despite having a visual element, the cleave is physical and inflicts {{buff(b="pvuln", dur=4)}},
   making the follow-up cleave lethal, but not the other attacks in this sequence.
...
matching the element they were hit by.
